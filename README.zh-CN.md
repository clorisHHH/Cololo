...
COLOLO 是一个免费、开源的桌面工具，用于在本地理解图片并重建结构化 Prompt。它帮助你研究视觉参考，同时不会假装一张成图能够准确还原原始模型、LoRA、采样器或 seed。
...
保持 Ollama 运行。模型可用时 COLOLO 会调用本地模型；模型不可用时，界面会明确提示。
...
COLOLO 是一个 local-first 的创作工具。模型权重、Ollama、Stable Diffusion、ControlNet 和 IP-Adapter 仍分别受各自许可证和使用条款约束。
